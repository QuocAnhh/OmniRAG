...
class AdvancedRAGService:
    ...
    def __init__(self):
        ...
        if settings.USE_LOCAL_EMBEDDINGS:
            ...
            self.embeddings = OpenAIEmbeddings(
                ...
                openai_api_base=self.base_url if self.ai_provider == "megallm" else None,
        ...
        self.llm_fast = self._create_llm(
            model_name=settings.DEFAULT_LLM_FAST_MODEL,
            temperature=0,
            max_tokens=500
        )
    ...
    def _get_api_key(self) -> str:
        ...
        if self.ai_provider == "megallm":
            if not settings.MEGALLM_API_KEY or settings.MEGALLM_API_KEY == "YOUR_MEGALLM_API_KEY_HERE":
                logger.warning("⚠️  MegaLLM API key not configured! Falling back to OpenAI provider.")
                logger.warning("   To use MegaLLM, get your API key from https://megallm.io/dashboard/overview")
    ...
    def _create_llm(self, model_name: str, temperature: float = 0.7, max_tokens: int = 500) -> ChatOpenAI:
        ...
        **prompt_kwargs
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.info(f"Successfully generated response with primary model (attempt {retry_count + 1})")
                return response
            except Exception as e:
                retry_count += 1
                last_error = e
        ...
        logger.warning(f"Primary model failed after {max_retries} attempts. Trying fallback models...")
    ...
    async def ingest_file(
        ...
        try:
            ...
            logger.info(f"Split into {len(chunks)} chunks using {chunking_strategy} strategy")
            ...
            for idx, (text, embedding, metadata) in enumerate(zip(texts, embeddings, metadatas)):
                point_id = hashlib.md5(f"{bot_id}:{file.filename}:{idx}".encode()).hexdigest()
                points.append(
                    PointStruct(
                        id=point_id,
                        vector=embedding,
...
Answer:"""
        ...
        avg_score = sum(doc.metadata.get("score", 0) for doc in reranked_docs) / len(reranked_docs)
