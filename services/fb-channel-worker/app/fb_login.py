"""
Facebook Login V2 — email/password + optional 2FA → session cookies.
Written by Nguyen Minh Huy (RainTee), adapted for OmniRAG FB worker.
Original: fbchat-v2/src/_core/_facebookLogin.py
"""
import string
import random
import pyotp
import httpx  # use httpx instead of requests (async-compatible)
...
def _post_json(url, data, headers):
    try:
        response = httpx.post(url, data=data, headers=headers, timeout=REQUEST_TIMEOUT)
...
def GetToken2FA(key2Fa):
    try:
        if not key2Fa:
            return ""
        return str(pyotp.TOTP(key2Fa.replace(" ", "")).now())
    except Exception:
        return str(random.randint(100000, 999999))


class loginFacebook:
    """Login to Facebook with email/password + optional 2FA. Returns session cookies."""
